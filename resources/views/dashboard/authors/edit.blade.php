...
@section('content')
@php
    $breadcrumb = 'Authors / Edit';
@endphp

<div class="bg-white rounded-lg shadow">
    <div class="px-6 py-4 border-b border-gray-200">
        <h1 class="text-2xl font-semibold text-gray-900">Edit Author: {{ $author->name }}</h1>
    </div>
        ...
        <form method="POST" action="{{ route('authors.update', $author) }}" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="biography" 
                          name="biography" 
                          rows="4"
                          class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('biography', $author->biography) }}</textarea>
                @error('biography')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
